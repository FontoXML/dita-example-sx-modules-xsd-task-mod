define([
	...
	'fontoxml-families/createLabelWidget',
	'fontoxml-families/createLabelQueryWidget',
	...
	return function configureSxModule (sxModule) {
		...
		// p in chdesc
		configureAsBlock(sxModule, 'self::p[parent::chdesc] and not(preceding-sibling::p or following-sibling::p)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the description')
		});
		...
		// p in chdeschd
		configureAsBlock(sxModule, 'self::p[parent::chdeschd] and not(preceding-sibling::p or following-sibling::p)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the title for the descriptions')
		});
		...
		configureAsDefinitionsTableRow(sxModule, 'self::chhead', t('header'), {
			columns: [
				{ query: './choptionhd', width: 1 },
				{ query: './chdeschd', width: 2 }
			],
			borders: true,
			backgroundColor: 'black',
			showWhen: 'always',
			contextualOperations: [
				{ name: ':contextual-delete-chhead' }
			]
		});
		...
		// p(only) in choice
		configureAsLine(sxModule, 'self::p[parent::choice] and not(preceding-sibling::p or following-sibling::p)', t('paragraph'), {
			emptyElementPlaceholderText: t('describe a choice for this step')
		});
		...
		// p in choption
		configureAsBlock(sxModule, 'self::p[parent::choption] and not(preceding-sibling::p or following-sibling::p)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the option')
		});
		...
		// p in choptionhd
		configureAsBlock(sxModule, 'self::p[parent::choptionhd] and not(preceding-sibling::p or following-sibling::p)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the title for the options')
		});
		...
		function getContextualOperationsForNoteType (noteType) {
			return NOTE_CONVERT_OPERATIONS
				...
				.concat([
					{ name: ':contextual-delete-note' }
				]);
		}
		configureContextualOperations(sxModule, 'self::note[parent::step or parent::substep]',
		...
		[
			...
			configureContextualOperations(sxModule, 'self::note[@type="' + noteType + '" and (parent::step or parent::substep)]',
		...
		configureAsGroup(sxModule, 'self::substep[parent::substeps[parent::step[parent::steps-unordered]]]', t('substep'), {
			defaultTextContainer: 'cmd',
			titleQuery: './cmd',
			blockBefore: [
				createLabelWidget('step')
		...
		configureAsFrame(sxModule, 'self::task[parent::*[fonto:dita-class(., "topic/topic")]]', undefined, {
			defaultTextContainer: 'taskbody',
			blockFooter: [
				createRelatedNodesQueryWidget('./related-links')
			],
